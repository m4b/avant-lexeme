...
From a given lexical description, we collect all the regular esxpressions, and then for each, we apply Thompson's algorithm, generate a dfa from the nfa (subset construction), and finally apply Hopcroft's minimization algorithm.

We then construct a Haskell module, which when suitably compiled with a parser, will accept and tokenize only those strings given by the original language description.  I.e., the scanner when imported as a module provides a function |scan| whose type is: |scan :: String -> [Token]|
...
writeImports moduleName = "module " 
   ++ moduleName ++ "(scan) where \n" ++ 
...
writeClasses classes = 
  "data Class = " ++ names ++ " deriving Show\n" where
  names = concat . intersperse 
   " | " . map (capitalize . name) $ classes

writeTokens :: String
writeTokens = "data Token = Token " ++
              "{kind :: Class, relevance :: Relevance," ++
              "location :: Location, token :: String}" ++
              " deriving Show\n"
...
writeDFA c = name' ++ " :: DFA' Char\n" ++
  ...
  dfa   = replace '"' "\\\"" . 
        show . hopcroft . 
        subsetConstruction . thompson . regex $ c
...
writeConstructors cs = "constructors :: " ++
              "[Location -> String -> Token]\n" ++
              "constructors = " ++ names ++ "\n" where
  names = replace '"' "" . 
   show . map ( (++ "'") . name) $ cs
...
writeScan = concat [
 ...
 "  scan' acc s = " ++
 "if done then reverse (t:acc) else reCURSE where\n",
 "    reCURSE = if err then error \"Could not parse.\""++
 " else scan' (t:acc) rem\n",
 "    done = rem == []\n",
 "    err = consumed == 0\n",
 "    list = zip dfas . map (tokenize s) $ dfas\n",
 "    (dfa,(consumed, token, rem)) = " ++ 
 "maximumBy compareTrip list\n",
...
scannerGenerator moduleName desc = program where
  ...
  program = writeImports moduleName ++
            writeClasses classes ++ writeTokens ++
            dfas ++ writeDFAs classes ++ constructors ++
            writeConstructors classes ++ writeScan
