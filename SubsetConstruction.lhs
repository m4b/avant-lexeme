...
findAccepting nfa labels = S.fromList sets where
  ...
  isAccepting label _ = S.empty /= (S.union accept label)
...
subsetConstruction' nfa next labels dfamap (s:ss) = if done then continue else recursion where
...
labelSets' next labels (s:ss) = if (M.member s labels) then (labelSets' next labels ss) else (labelSets' (next+1) (M.insert s next labels) ss)
...
edgeMap = edges' M.empty where
  ...
  edges' acc _ [] = acc
  edges' acc labels ((alpha, set):ss) = edges' acc' labels ss where
